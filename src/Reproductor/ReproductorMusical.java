...
public class ReproductorMusical extends JFrame {
    ...
    public ReproductorMusical(SessionManager sessionManager) {
        ...
        returnButton.addActionListener(evt -> returnToWindows());
    ...
    private void returnToWindows() {
        // Regresar a la ventana de Windows
        Windows windows = new Windows(sessionManager);
        windows.setVisible(true);
        dispose(); // Cierra el reproductor musical
    }
    ...
    private void addSong() {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            if (imageResult == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    // Obtener la ruta de la carpeta "Música" del usuario logueado
                    String musicaFolderPath = sessionManager.getCurrentUserMusicFolderPath();
                    if (musicaFolderPath == null) {
                        showErrorDialog("No se pudo obtener la ruta de la carpeta Música del usuario.", ERROR_MESSAGE_TITLE);
                        return;
                    }

                    File destinoArchivo = new File(musicaFolderPath, selectedAudioFile.getName());

                    // Copiar el archivo a la carpeta de Música del usuario
                    Files.copy(selectedAudioFile.toPath(), destinoArchivo.toPath(), StandardCopyOption.REPLACE_EXISTING);

                    ImageIcon icon = getScaledImageIcon(selectedImageFile, 150, 150);
                    actualizarImagenCancion(icon);

                    Cancion nuevaCancion = new Cancion(selectedAudioFile.getName(), artista, duracion, icon, AUDIO_FORMAT, destinoArchivo.getAbsolutePath());
                    ...
                    showErrorDialog("Error al cargar la imagen o copiar el archivo de audio.", ERROR_MESSAGE_TITLE);
                    ex.printStackTrace();
                }
            }
        }
